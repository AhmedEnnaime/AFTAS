...
export class RankingsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.store.dispatch(
      findCompetition({ competitionCode: this.competitionCode })
    );
    this.store.dispatch(
      rankingPageActoins.loadCompetitionRankings({
        competitionCode: this.competitionCode,
      })
    );
    this.store
      .select(selectFoundedCompetition)
  ...
  isCompetitionDatePassed(): boolean {
    return (
      new Date().getTime() > new Date(this.competition?.endTime ?? '').getTime()
    );
  }

  isCompetitionOngoing() {
    let now = new Date().getTime();
    let start = new Date(this.competition?.startTime ?? '').getTime();
    let end = new Date(this.competition?.endTime ?? '').getTime();
  ...
  setUpCompetitionRankings() {
    this.store.dispatch(
      rankingPageActoins.setCompetitionRanking({
        competitionCode: this.competitionCode,
      })
    );
  }
